...
local VariantCfg = require(Configs:WaitForChild("CrewmateVariants"))
...
for _, info in pairs(CrewmatesData) do
	...
	if variantKey ~= "Normal" then
		...
		if v then
			...
			for _, baseId in ipairs(baseIds) do
				local baseInfo = CrewmatesData[baseId]
				if baseInfo and type(baseInfo) == "table" then
					local newId = prefix .. baseId
					if CrewmatesData[newId] == nil then
