...
local ReactModule = Packages:WaitForChild("React") :: ModuleScript
local React = require(ReactModule)

local Modules = ReplicatedStorage:WaitForChild("Modules")
local DevilFruitAssets = require((Modules:WaitForChild("DevilFruits"):WaitForChild("Assets")) :: ModuleScript)
local ChestVisuals = require((Modules:WaitForChild("GrandLineRushChestVisuals")) :: ModuleScript)
...
local function inventoryToggleButton(props)
	...
	local zIndexBase = props.zIndexBase or 0
	local iconPosition = compact and UDim2.fromOffset(15, 36) or UDim2.fromScale(0.5, 0.44)
	local iconSize = compact and UDim2.fromOffset(40, 40) or UDim2.fromOffset(34, 34)
	local toggleIcon = props.toggleIcon or {}
	local hasLegacyIcon = typeof(toggleIcon.image) == "string" and toggleIcon.image ~= ""
	local toggleLabel = props.toggleLabel or "Inventory"
	local keyText = props.keyText or ""

	local iconChild
	if hasLegacyIcon then
		local imageProps = {
			AnchorPoint = compact and Vector2.new(0, 0.5) or Vector2.new(0.5, 0.5),
			BackgroundTransparency = 1,
	...
	return e("TextButton", mergeProps({
		...
		BackgroundColor3 = compact and Color3.fromRGB(42, 44, 49) or PALETTE.InkSoft,
		BackgroundTransparency = compact and 0.18 or 0.5,
		BorderSizePixel = 0,
		Position = position,
		ref = hoverRef,
		Size = size,
		...
		Scale = e("UIScale", {
			Scale = (hovered and (compact and 1.025 or 1.018) or 1) - (pressed and 0.016 or 0),
		}),
		Corner = e("UICorner", {
			CornerRadius = UDim.new(0, compact and 4 or 16),
		}),
		Gradient = not compact and e("UIGradient", {
			Rotation = 90,
			Color = ColorSequence.new({
		...
		Stroke = e("UIStroke", {
			Color = compact and Color3.fromRGB(22, 24, 28) or Color3.fromRGB(72, 98, 146),
			Transparency = hovered and (compact and 0 or 0.08) or (compact and 0.06 or 0.18),
			Thickness = compact and (hovered and 2.2 or 2) or (hovered and 1.7 or 1.4),
		}),
		Shadow = not compact and e("UIStroke", {
			Color = PALETTE.Sea,
			Transparency = hovered and 0.84 or 0.95,
			Thickness = 2,
		}),
		Inset = not compact and e("Frame", {
			BackgroundColor3 = Color3.fromRGB(7, 11, 20),
		...
		Icon = iconChild,
		Title = not compact and e("TextLabel", {
		...
		KeyLabel = compact and keyText ~= "" and e("TextLabel", {
			BackgroundTransparency = 1,
			Font = Enum.Font.GothamBlack,
		...
		CompactLabel = compact and keyText == "" and e("TextLabel", {
...
local function captainsLogRow(props)
	...
	return e("Frame", {
		...
		Subtitle = e("TextLabel", {
			...
			Text = string.format("%s  |  %s", tostring(entry.subtitle or "Crewmate"), tostring(entry.standName or "Roster")),
		...
		Footer = e("TextLabel", {
			...
			Text = string.format("Current XP: %s", formatNumber(entry.collectable or 0)),
			TextColor3 = accent,
			TextSize = 12,
			TextTruncate = Enum.TextTruncate.AtEnd,
			TextXAlignment = Enum.TextXAlignment.Left,
...
local function App(props)
	...
	local shopLayout = props.shopLayout or {}
	local dockShopLeft = shopLayout.dock == "hotbarLeft"
	local shopWidth = dockShopLeft and ((shopLayout.size and shopLayout.size.X.Offset) or 74) or 0
	local shopGap = dockShopLeft and 12 or 0
	...
	local launcherWidth = toggleWidth
	if dockShopLeft then
		launcherWidth += toggleGap + shopGap + shopWidth
	elseif dockToggleLeft and toggleWidth > 0 then
		launcherWidth += toggleGap
	end
	local bottomBarWidth = dockToggleLeft and (launcherWidth + hotbarWidth) or hotbarWidth
	...
	local children = {
		BottomBar = e("Frame", {
			...
			Toggle = e(inventoryToggleButton, {
				...
				keyText = props.toggleKeyText,
				zIndexBase = 10,
				onToggle = props.onToggle,
			}),
			ShopToggle = dockShopLeft and e(inventoryToggleButton, {
				toggleLayout = {
					anchorPoint = Vector2.new(0, 0),
					position = UDim2.fromOffset(toggleWidth + toggleGap, 18),
					size = shopLayout.size,
					compact = shopLayout.compact,
				},
				toggleIcon = props.shopIcon,
				toggleLabel = props.shopLabel or "Shop",
				keyText = props.shopKeyText,
				zIndexBase = 10,
				onToggle = props.onShopToggle,
			}) or nil,
			Hotbar = e("Frame", {
				BackgroundTransparency = 1,
				Position = UDim2.fromOffset(dockToggleLeft and launcherWidth or 0, 0),
			...
			Shell = e("Frame", {
				...
				MainPanel = e("Frame", {
					...
					SearchShell = e("Frame", {
						...
						PlaceholderText = showingCaptainLog and "Search crewmates..." or "Search inventory...",
