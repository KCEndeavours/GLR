...
local function getDecreasePart()
	local decreasePart = Workspace:FindFirstChild("DecreaseSpeed")
	if decreasePart and decreasePart:IsA("BasePart") then
		return decreasePart
	end
...
local function getSpeedValue(player)
	...
	local speedObj = hidden:FindFirstChild("Speed")
	if speedObj and speedObj:IsA("NumberValue") then
		return speedObj
	end
...
local function hookCharacter(player, character)
	...
	local speedObj = getSpeedValue(player)
	local decreasePart = getDecreasePart()
	...
	local function getUnboostedSpeed()
		...
		return base + (speedObj and speedObj.Value or 0)
	end
	...
	conns[#conns + 1] = humanoid:GetPropertyChangedSignal("WalkSpeed"):Connect(function()
		...
		if humanoid.WalkSpeed ~= expected then
			...
			base = normalizedSpeed - (speedObj and speedObj.Value or 0)
			apply()
		end
	end)
	...
	if decreasePart then
		conns[#conns + 1] = decreasePart.Touched:Connect(function(hit)
		...
		conns[#conns + 1] = decreasePart.TouchEnded:Connect(function(hit)
			if not isCharacterPart(hit) then return end
		...
		conns[#conns + 1] = RunService.Heartbeat:Connect(function(dt)
			...
			for _, part in ipairs(decreasePart:GetTouchingParts()) do
